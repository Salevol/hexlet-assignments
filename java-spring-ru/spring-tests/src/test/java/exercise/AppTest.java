...
public class AppTest {
    ...
    void testUpdatePerson() throws Exception {
        MockHttpServletResponse responsePatch = mockMvc
                .perform(patch("/people/5")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"firstName\": \"Jonny\", \"lastName\": \"Depp\"}"))
                .andReturn()
                .getResponse();
        ...
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
        // Проверяем, что созданная сущность появилась в базе
        assertThat(response.getContentAsString()).contains("Jonny", "Depp");
    }
    ...
    void testDeletePerson() throws Exception {
        ...
        assertThat(response.getContentType()).isEqualTo(MediaType.APPLICATION_JSON.toString());
        // Проверяем, что созданная сущность появилась в базе
        assertThat(response.getContentAsString()).doesNotContain("Jonny", "Depp");
    }
    // END
}
